...
class DashboardStore:
    ...
    def start_execution(self, job: dict, worker_id: str, *, started_at: str | None = None) -> dict:
        ...
        with self.backend.transaction() as db:
            _execute(
                ...
                (ident, job["key"], payload.get("workflow_id"), payload.get("workflow_task_id"),
                 job["repository"], worker_id, attempt, "running", at, at),
            )
    ...
    def update_live_execution(self, job_key: str, worker_id: str, telemetry: dict, *, at: str | None = None) -> dict:
        ...
        with self.backend.transaction() as db:
            ...
            if row is None or row["status"] != "running":
                raise KeyError("running execution not found")
            ...
            _execute(
                ...
                """UPDATE job_executions
                   SET current_stage=?, progress_percent=?, live_usage_json=?, last_telemetry_at=?
                   WHERE id=?""",
                (stage, progress if progress is not None else old,
                 json.dumps(live_usage, sort_keys=True), at or _now(), row["id"]),
            )
            return self._fetchone(db, "SELECT * FROM job_executions WHERE id=?", (row["id"],))

    def finish_execution(
        self, job_key: str, worker_id: str, *, status: str,
        duration_seconds: float | None, result: dict | None,
        ...
        with self.backend.transaction() as db:
            ...
            _execute(
                db, self.backend,
                """UPDATE job_executions SET status=?, finished_at=?, duration_seconds=?,
                   api_calls=?, input_tokens=?, cached_input_tokens=?, output_tokens=?,
                   reasoning_tokens=?, total_tokens=?, commit_count=?, commit_shas_json=?,
                   error_type=?, error_message=?, result_summary_json=? WHERE id=?""",
                (status, finished_at or _now(), duration_seconds,
                 ...
                 int(usage.get("reasoning_tokens") or 0), int(usage.get("total_tokens") or 0),
                 len(shas), json.dumps(shas), error_type, error_message,
            ...
            for index, item in enumerate(providers):
                _execute(
                    ...
                    """INSERT INTO api_usage_events(
                       id, execution_id, worker_id, repository, provider, model, api_calls,
                    ...
                    (f"{row['id']}:{index}", row["id"], worker_id, row["repository"],
                     ...
                     item.get("estimated_cost_usd"), item.get("pricing_catalog_version"), finished_at or _now()),
                )
            return self._fetchone(db, "SELECT * FROM job_executions WHERE id=?", (row["id"],))

    def executions_for_worker(self, worker_id: str, *, limit: int = 100) -> list[dict]:
        with self.backend.connect() as db:
            return self._fetchall(db, "SELECT * FROM job_executions WHERE worker_id=? ORDER BY started_at DESC LIMIT ?", (worker_id, max(1, min(limit, 500))))

    def executions_for_repository(self, repository: str, *, limit: int = 100) -> list[dict]:
        with self.backend.connect() as db:
            return self._fetchall(db, "SELECT * FROM job_executions WHERE repository=? ORDER BY started_at DESC LIMIT ?", (repository, max(1, min(limit, 500))))
    ...
    def usage_events(self, *, worker_id=None, repository=None, since=None) -> list[dict]:
        ...
        with self.backend.connect() as db:
            return self._fetchall(db, "SELECT * FROM api_usage_events" + where + " ORDER BY occurred_at DESC", params)
    ...
    def append_logs(self, worker_id: str, rows: list[dict]) -> list[dict]:
        ...
        with self.backend.transaction() as db:
            for index, source in enumerate(rows):
                ...
                (ident, worker_id, row.get("repository"), row.get("workflow_id"), row.get("job_key"),
                 row.get("level") or "info", row.get("stage"), str(row.get("message") or ""),
                 row.get("provider"), row.get("model"), json.dumps(row.get("metadata") or {}, sort_keys=True), created))
    ...
    def logs_for_worker(self, worker_id: str, *, after: str | None = None, limit: int = 100) -> list[dict]:
        ...
        with self.backend.connect() as db:
            if after is None:
                return self._fetchall(db, "SELECT * FROM worker_log_events WHERE worker_id=? ORDER BY created_at DESC, id DESC LIMIT ?", (worker_id, limit))
            ...
            return self._fetchall(db, """SELECT * FROM worker_log_events WHERE worker_id=?
                AND (created_at < ? OR (created_at = ? AND id < ?))
                ORDER BY created_at DESC, id DESC LIMIT ?""",
                (worker_id, cursor["created_at"], cursor["created_at"], after, limit))
    ...
    def _save_snapshot(self, table: str, snapshot: dict, json_fields: tuple[str, ...]) -> dict:
        ...
        for field in json_fields:
            values[field] = json.dumps(values.get(field) or ([] if field in {"remaining_work_json","blockers_json"} else {}), sort_keys=True)
    ...
    def save_repository_snapshot(self, snapshot: dict) -> dict:
        return self._save_snapshot("project_repository_snapshots", snapshot, ("snapshot_json",))
    ...
    def latest_provider_quota_snapshots(self) -> list[dict]:
        with self.backend.connect() as db:
            return self._fetchall(db, """SELECT q.* FROM provider_quota_snapshots q
                JOIN (SELECT provider, MAX(captured_at) captured_at FROM provider_quota_snapshots GROUP BY provider) latest
                ON latest.provider=q.provider AND latest.captured_at=q.captured_at ORDER BY q.provider""")
